...
export const createTicket = async (req: Request) => {
  try {
    const { title, description, urgency, id_user } = await req.json()
    const insert = db.prepare(`
      INSERT INTO tickets (title, description, urgency, id_user)
      VALUES (?, ?, ?, ?)
      RETURNING *
    `)
    const result = insert.get(title, description, urgency, id_user)
